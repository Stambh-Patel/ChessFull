...
/* ============================================================
   ...
   ------------------------------------------------------------
   IMPORTANT ARCHITECTURE NOTE: this app runs on serverless
   functions (e.g. Netlify), where consecutive requests can hit
   different, memoryless function instances. So the game's
   position/history is NOT stored server-side - the CLIENT holds
   the full ply-by-ply history (`plies` below) and sends the
   current FEN with every API call. Every API route is a pure
   function of the FEN (and move) it's given. Nothing rendered
   here is fabricated: board state, legal moves, evaluation
   numbers, node counts, and the search tree all come straight
   from the API's JSON responses.
   ============================================================ */
...
const DATA_STRUCTURES = [
  ...
  },
  {
    ...
    how: 'Immutable, like Move and Piece: moveExecutor.ts\'s applyMove() takes one GameState and a Move and returns a brand-new GameState - which is also exactly why this frontend can safely hold a whole array of them as its undo/redo history.',
  },
  {
...
export default function ChessLab() {
  ...
  // ------------------------------------------------------------------
  // Core play state - held CLIENT-SIDE. `plies[0]` is the starting
  // position; `plies[i]` is the position after ply i. `plyMoves[i-1]`
  // is the move that produced `plies[i]`. currentPly indexes both,
  // which is what makes Undo/Redo/goto instant and purely local.
  // ------------------------------------------------------------------
  const [plies, setPlies] = useState<GameStateDto[]>([]);
  const [plyMoves, setPlyMoves] = useState<MoveDetail[]>([]);
  const [currentPly, setCurrentPly] = useState(0);

  const current: GameStateDto | null = plies.length > 0 ? plies[currentPly] : null;
  const boardGrid = current ? current.board : null;
  const sideToMove = current ? current.sideToMove : 'WHITE';
  const status = current ? current.status : 'ONGOING';
  const fen = current ? current.fen : '';
  ...
  const buttonsDisabled = status === 'CHECKMATE' || status === 'STALEMATE';
  ...
  const calculateBestMove = useCallback(async (playIt: boolean) => {
    ...
    const result: SearchResultDto = await apiPost('/engine/best-move', { fen: cur.fen, depth, algorithm });
  ...
  async function attemptMove(from: string, to: string, promotionType: string | null) {
    ...
    const body: Record<string, string> = { fen: cur.fen, from, to };
    ...
    stepPanelAdd('STEP 7', `Game state updated: ${result.status === 'ONGOING' ? oppositeColor(cur.sideToMove) + ' to move' : result.status}`);

    setLastMove({ from: result.from, to: result.to });
    clearSelection();

    const newDto: GameStateDto = await apiGet(`/game/state?fen=${encodeURIComponent(result.fen)}`);
    const moveDetail: MoveDetail = {
      uci: result.move, from: result.from, to: result.to, piece: result.piece,
      type: result.type, isCapture: result.isCapture,
    };

    setPlies((prev) => [...prev.slice(0, currentPly + 1), newDto]);
    setPlyMoves((prev) => [...prev.slice(0, currentPly), moveDetail]);
    setCurrentPly((p) => p + 1);

    if (newDto.status === 'CHECKMATE') {
      showToast(`Checkmate. ${cur.sideToMove === 'WHITE' ? 'White' : 'Black'} wins.`);
    } else if (newDto.status === 'STALEMATE') {
      showToast('Stalemate. The game is drawn.');
    } else if (newDto.status === 'CHECK') {
      stepPanelAdd('CHECK', `${newDto.sideToMove}'s king is in check.`);
    }
  ...
  async function onSquareClick(algebraic: string) {
    ...
    if (code) {
      ...
      const data = await apiGet(`/game/legal-moves?fen=${encodeURIComponent(current.fen)}&square=${algebraic}`);
  ...
  async function onNewGame() {
    ...
    const dto: GameStateDto = await apiPost('/game/new');
    setPlies([dto]);
    setPlyMoves([]);
    setCurrentPly(0);
    stepPanelClear();
  ...
  function onUndo() {
    if (currentPly === 0) { showToast('Nothing to undo - already at the start of the game.'); return; }
    clearSelection();
    setCurrentPly((p) => p - 1);
  }

  function onRedo() {
    if (currentPly >= plies.length - 1) { showToast('Nothing to redo - already at the latest move.'); return; }
    clearSelection();
    setCurrentPly((p) => p + 1);
  }
  ...
  function gotoPly(ply: number) {
    if (ply < 0 || ply >= plies.length) return;
    clearSelection();
    setCurrentPly(ply);
  }
  ...
  const fenFields = fen ? fen.split(' ') : null;

  function refreshFenTab() {
    if (current) setFenInput(current.fen);
  }
  ...
  async function loadFenIntoGame() {
    ...
    clearSelection();
    setPlies([result as GameStateDto]);
    setPlyMoves([]);
    setCurrentPly(0);
  }
  ...
  const refreshBoardRepTab = useCallback(async () => {
    ...
    const data = await apiGet(`/board?fen=${encodeURIComponent(currentRef.current.fen)}`);
  ...
  async function runEngineSearch() {
    ...
    const data: SearchResultDto = await apiGet(`/engine/search?fen=${encodeURIComponent(current.fen)}&depth=${engineDepth}&algorithm=${engineAlgo}&treeDepth=2`);
          ...
          <div className="play-layout">
            ...
            <div className="right-rail">
              ...
              <div className="panel">
                ...
                <MoveList moves={plyMoves} currentPly={currentPly} onGotoPly={gotoPly} />
              </div>
            </div>
          </div>
        ...
        <section className="tab-panel">
          <div className="two-col">
            <div className="panel">
              ...
              <p className="muted" style={{ marginTop: '-6px' }}>This is the actual grid computed for the current position, fetched from <code>GET /api/board?fen=...</code>.</p>
